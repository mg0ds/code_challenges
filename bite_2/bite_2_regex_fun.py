...
def get_all_hashtags_and_links(tweet=TWEET):
    """Get all hashtags and links from the tweet text
       ...
       Return this list.
    """
    wiersz = re.split(r"\s", tweet)
    list = []
    for w in wiersz:
        if re.match(r"http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+", w) != None or re.match(r"(#\w+)", w):
            list.append(w)
    return list
