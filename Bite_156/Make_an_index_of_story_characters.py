...
def make_character_index(text=text, characters=CHARACTERS):
    ...
    """
    index_dict = {}
    def char_index(char, row_number):
        if char in index_dict:
            index_dict[char].append(row_number)
            index_dict[char] = sorted(list(set(index_dict[char])))
        else:
            index_dict[char] = [row_number]
    ...
    for count, item in enumerate(rows):
        ...
        for char in characters:
            if type(char) is tuple:
                for char2 in char:
                    if char2.lower() in item.lower():
                        #print(char[0])
                        char_index(char[0].lower(), count)
            else:
                if char.lower() in item.lower():
                    char_index(char.lower(), count)
    return index_dict
