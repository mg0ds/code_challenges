...
def max_and_min_years_new_characters(characters=characters):
    ...
    for char in characters:
        if char.year != "":
            year_list.append(char.year)
    t = (Counter(year_list).most_common()[0][0], Counter(year_list).most_common()[-1][0])
    return t
...
def get_percentage_female_characters(characters=characters):
    ...
    for char in characters:
        if char.sex != "":
            list.append(char.sex)
    return(round(((int(Counter(list).most_common()[1][1]) / int(len(list))) * 100), 2))
