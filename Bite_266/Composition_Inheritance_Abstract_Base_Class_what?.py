...
class Site(ABC):
    ...
    def find_table(self, loc: int = 0) -> str:
        ...
        Returns:
            str -- The html table
        """
        table_list = self.web.soup.find_all("table")
        return table_list[loc]
...
class RealClearPolitics(Site):
    ...
    def parse_rows(self, table: Soup) -> List[Poll]:
        ...
        for row in tr_list:
            if i != 7:
                try:
                    row_list[j].append(float(row.get_text()))
                except:
                    if row.get_text() == "--":
                        row_list[j].append(0)
                    else:
                        row_list[j].append(row.get_text())
                i += 1
            else:
                j += 1
                i = 1
                row_list.append([row.get_text()])
        
        for row in row_list[1:]:
            poll_name.append(Poll._make(row))
        #print(row_list)
        return poll_name
...
class NYTimes(Site):
    ...
    def parse_rows(self, table: Soup) -> List[LeaderBoard]:
        ...
        i = 0
        j = 0
        for row in td_list[:15]:
            if i != 5:
                if i == 4:
                    row_list[j].append(int(row.get_text().strip()[1]))
                    i += 1
                else:
                    try:
                        row_list[j].append(int(row.get_text().strip()))
                        i += 1
                    except:
                        row_list[j].append(row.get_text().strip())
                        i += 1
            else:
                j += 1
                i = 1
                row_list.append([row.get_text().strip()])
        for row in row_list:
            leaderb_list.append(LeaderBoard._make(row))
        return leaderb_list
