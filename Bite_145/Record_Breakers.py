...
def high_low_record_breakers_for_2015():
    ...
    df["Data_Value"] = df["Data_Value"].divide(10)
    
    df2015 = df[(df['Date'] > '2015-01-01') & (df['Date'] < '2015-12-31')]
    df2015S = df2015.nsmallest(1, "Data_Value")
    df2015L = df2015.nlargest(1, "Data_Value")
    high_2015 = STATION(df2015L["ID"].array, df2015L["Date"].array, df2015L["Data_Value"].array)
    low_2015 = STATION(df2015S["ID"].array, df2015S["Date"].array, df2015S["Data_Value"].array)
    bbbb = (high_2015, low_2015)
